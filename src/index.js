    ...
    try {
        ...
        rl.on('line', async (userInput)=>{
            try {
                ...
                switch (command) {
                    case "up": {
                        currentDirectory = upOneLevel(rl, currentDirectory)
                        break;
                    }
                    case "cd": {
                        cd(lineArgs[1], currentDirectory, rl).then(newDirectory => {
                            currentDirectory=newDirectory
                        });
                        break;
                    }
                    case "ls": {
                        ls(currentDirectory, rl);
                        break;
                    }
                    case "cat": {
                        await cat(currentDirectory, lineArgs[1], rl);
                        break;
                    }
                    case "add": {
                        add(currentDirectory, lineArgs[1], rl);
                        break;
                    }
                    case "rn": {
                        rn(currentDirectory, lineArgs[1], lineArgs[2], rl);
                        break;
                    }
                    case "cp": {
                        cp(currentDirectory, lineArgs[1], lineArgs[2], rl);
                        break;
                    }
                    case "mv": {
                        mv(currentDirectory, lineArgs[1], lineArgs[2], rl);
                        break;
                    }
                    case "rm": {
                        rm(currentDirectory, lineArgs[1], rl);
                        break;
                    }
                    case "os": {
                        ...
                        switch (osCommand) {
                            ...
                            case "cpus": {
                                sendInfoMessage(`System cpus - ${os.cpus().length}\nDetailed info:\n${os.cpus().reduce((acc, cpu)=>{
                    ...
                    case "hash": {
                        await calcHash(currentDirectory, lineArgs[1], rl);
                        break;
                    }
                    case "compress": {
